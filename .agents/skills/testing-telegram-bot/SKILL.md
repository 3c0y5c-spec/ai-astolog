...
description: Test the ai-astolog Telegram bot runtime flows end-to-end. Use when verifying Telegram command handling, onboarding questionnaires, chart summaries, or bot runtime changes.
---
...
3. Stop any existing polling containers before starting a new one. Telegram allows only one active `getUpdates` poller per bot token:
   ```bash
   docker rm -f ai-astolog-chart-test ai-astolog-profile-test ai-astolog-profile-e2e 2>/dev/null || true
   ```
4. Start the bot in polling mode without putting the secret value in the command line:
   ```bash
   docker run --rm --name ai-astolog-chart-test \
     -p 18081:8080 \
